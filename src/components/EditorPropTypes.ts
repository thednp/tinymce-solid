...
export type InitOptions =
  ...
  & {
    ...
    readonly?: DoNotUse<
      "readonly prop is overridden by the component, use the `disabled` prop instead"
    >;
    license_key?: DoNotUse<
      "license_key prop is overridden by the integration, use the `licenseKey` prop instead"
    >;
  }
  & { [key: string]: unknown };

export type Version = `${"4" | "5" | "6" | "7"}${
  | ""
  | "-dev"
  | "-testing"
  | `.${number}`
